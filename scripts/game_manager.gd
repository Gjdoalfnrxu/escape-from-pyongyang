...
func _on_game_started() -> void:
	...
	for pid in network.players:
		var pos: Vector2 = Vector2(200, 200) if spawn_points.is_empty() else spawn_points[i % spawn_points.size()].global_position
...
func on_player_reached_extraction(player_id: int) -> void:
	if player_id in alive_players:
		var player: CharacterBody2D = alive_players[player_id]
...
func _check_game_over() -> void:
	if alive_players.is_empty():
		...
		var winner: int = extracted_players[0]["id"] if not extracted_players.is_empty() else -1
